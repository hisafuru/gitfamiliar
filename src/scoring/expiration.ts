...
function parseDays(s: string): number {
  ...
  if (!match) throw new Error(`Invalid duration format: "${s}". Expected format like "180d".`);
  return parseInt(match[1], 10);
}

function parsePercentage(s: string): number {
  const match = s.match(/^(\d+)%$/);
  if (!match) throw new Error(`Invalid percentage format: "${s}". Expected format like "50%".`);
...
export async function isExpired(
  ...
  if (config.policy === 'time' || config.policy === 'combined') {
    ...
    if (lastTouch && config.duration) {
      const daysSince = (currentDate.getTime() - lastTouch.getTime()) / (1000 * 60 * 60 * 24);
  ...
  for (let i = 0; i < files.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(async (filePath) => ({
        path: filePath,
        expired: await isExpired(gitClient, filePath, user, config),
      })),
    );
    for (const { path, expired: isExp } of results) {
      if (isExp) expired.add(path);
    }
  }
